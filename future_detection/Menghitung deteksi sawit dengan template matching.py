...
sawit = cv2.imread('gambar2.jpeg')
sawit = cv2.cvtColor(sawit, cv2.COLOR_BGR2RGB)
# panggil dan konversi warna agar sesuai dengan Matplotlib
kebun_sawit = cv2.imread('gambar1.jpeg')
kebun_sawit = cv2.cvtColor(kebun_sawit, cv2.COLOR_BGR2RGB) 
plt.subplot(121),plt.imshow(sawit), plt.title('Dendi')
plt.subplot(122),plt.imshow(kebun_sawit), plt.title('Dendi dan Cokro dalam ruangan')
plt.show()

## membaca gambar utuh untuk dicari
img_rgb = cv2.imread('gambar1.jpeg')
img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
## membaca template 
template = cv2.imread('gambar2.jpeg',0)
