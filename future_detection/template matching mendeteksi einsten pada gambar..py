...
einstein = cv2.imread('gambar2.jpeg')
einstein = cv2.cvtColor(einstein, cv2.COLOR_BGR2RGB) # simpan dengan nama yang 
#sama = ditumpuk

# panggil dan konversi warna agar sesuai dengan Matplotlib
solvay = cv2.imread('gambar1.jpeg')
solvay = cv2.cvtColor(solvay, cv2.COLOR_BGR2RGB) 

plt.subplot(121),plt.imshow(einstein), plt.title('Dendi')
plt.subplot(122),plt.imshow(solvay), plt.title('Dendi dan cokro')
plt.show()
